...
from rlstructures import RL_Batcher, RL_Agent, DictTensor,Trajectories,TemporalDictTensor, masked_dicttensor
...
class MyAgent(RL_Agent):
    ...
    def __call__(self,state,observation,agent_info=None,history=None):
        ...
        state=masked_dicttensor(state,self.initial_state(agent_info,B),observation["initial_state"])
...
if __name__=="__main__":
    ...
    print("Initial state = ",trajectories.trajectories["observation/initial_state"][:,0])

    #The t+1 state of the last timestep are surely final states
    print("Done =",trajectories.trajectories["_observation/done"][torch.arange(B),lengths-1])
    ...
    slot_batcher.execute()
    trajectories,n=slot_batcher.get(blocking=True)
    print("The initial state of the running agent in this slot is ",trajectories.info.truncate_key("agent_state/"))
    print("Number of still running environments : ",n)

    slot_batcher.execute()
    trajectories,n=slot_batcher.get(blocking=True)
    print("The trajectories contain the execution of ",trajectories.info.n_elems()," agents")
    print("The initial state of the running agent in this slot is ",trajectories.info.truncate_key("agent_state/"))
    print("Number of still running environments : ",n)
    slot_batcher.close()
    del(slot_batcher)
