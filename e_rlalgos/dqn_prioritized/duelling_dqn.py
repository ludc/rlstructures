    ...
    def run(self):
        ...
        while time.time()-_start_time <self.config["time_limit"]:
            ...
            for k in range(self.config["qvalue_epochs"]):
                ...
                #print("-- MAX ",self.replay_buffer.priorities.max())
                self.logger.add_scalar("Max priority",self.replay_buffer.priorities.max().item(),self.iteration)
                dt = self.get_loss(transitions,device)

                [self.logger.add_scalar(k,dt[k].mean().item(),self.iteration) for k in dt.keys()]
