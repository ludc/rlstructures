...
class Reinforce:
    ...
    def run(self):
        ...
        optimizer = torch.optim.RMSprop(self.learning_model.parameters(), lr=self.config["lr"])
        ...
        while(time.time()-_start_time<self.config["time_limit"]):
            ...
            if not evaluation_trajectories is None: #trajectories are available
                ...
                self.logger.add_scalar("evaluation_reward/"+self.config["evaluation_mode"],cumulated_reward.item(),self.evaluation_iteration)
